...
import { deriveAccount, generateMnemonic, isValidMnemonic, walletFromPrivateKey, } from '@/lib/wallet-utils';
...
async function setAutoLockAlarm() {
...
function rateLimitOrigin(origin) {
    if (!origin)
        return;
    const now = Date.now();
...
async function submitSerialized(account, fn) {
    ...
    try {
        ...
        release();
        if (submissionQueues.get(account) === next)
            submissionQueues.delete(account);
    }
}
// ───────────────────── helpers ───────────────────────────────────────────
...
function ensureFiniteNumber(s, label) {
    const n = parseFloat(s ?? '');
    if (!Number.isFinite(n))
        throw new Error(`Invalid ${label}: not a number`);
    return n;
}
function findAccount(addressOrId) {
    if (!state.unlocked)
        throw new Error('Wallet is locked');
    const lower = addressOrId.toLowerCase();
    const found = state.unlocked.accounts.find((a) => a.id === addressOrId || a.address.toLowerCase() === lower);
    if (!found)
        throw new Error('Account not in vault');
    return found;
}
...
function newDerivedAccount(name) {
...
async function handle(req, sender) {
    ...
    switch (req.type) {
        ...
        case 'vault.create.mnemonic': {
            if (!isValidMnemonic(req.mnemonic))
        ...
        case 'settings.set': {
            ...
            if (req.settings.autoLockMinutes !== undefined)
                await setAutoLockAlarm();
            return next;
        }
        ...
        case 'evm.send.native': {
            ...
            try {
                return await submitSerialized(acct.address, () => sendNative(settings.network, acct.privateKey, req.to, req.amount));
            }
            catch (e) {
        ...
        case 'evm.send.erc20': {
            ...
            if (n <= 0)
                throw new Error('Amount must be positive');
            try {
                return await submitSerialized(acct.address, () => sendErc20(settings.network, acct.privateKey, req.token, req.to, req.amount));
            }
            catch (e) {
                throw new Error(friendlyError(e));
            }
            ...
            try {
                return await submitSerialized(acct.address, async () => {
                    ...
                    if (swap.status !== 'success')
                        throw new Error('Swap reverted on-chain');
        ...
        case 'evm.sign.tx': {
            ...
            try {
                return await submitSerialized(acct.address, () => signGenericTransaction(settings.network, acct.privateKey, normalizeTxRequest(tx)));
            }
            catch (e) {
                throw new Error(friendlyError(e));
            }
        ...
        case 'dapp.signTx': {
            ...
            if (!sim.ok) {
                warnings.push(sim.revertReason
...
function normalizeTxRequest(tx) {
    ...
    for (const k of ['value', 'gas', 'gasLimit', 'gasPrice', 'maxFeePerGas', 'maxPriorityFeePerGas', 'nonce', 'chainId']) {
        ...
        if (typeof v === 'string') {
            try {
                out[k] = v.startsWith('0x') ? BigInt(v) : BigInt(v);
            }
            catch { /* leave as-is */ }
        }
    }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    (async () => {
        try {
            ...
            const reply = { ok: false, error: safeError(e) };
            sendResponse(reply);
        }
    })();
    return true;
