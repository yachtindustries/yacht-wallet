...
export function assessTxRisk(tx) {
    const warnings = [];
    let isHighRisk = false;
    const data = typeof tx?.data === 'string' ? tx.data.toLowerCase() : '0x';
    const value = (() => {
        const v = tx?.value;
        if (v == null)
            return 0n;
...
export function looksHomograph(host) {
...
export function passwordStrength(pw) {
    ...
    if (/[0-9]/.test(pw) && /[^A-Za-z0-9]/.test(pw))
        score++;
    if (/^(?:password|qwerty|letmein|admin|welcome|123456)/i.test(pw))
        score = 0;
    if (/(.)\1{4,}/.test(pw))
