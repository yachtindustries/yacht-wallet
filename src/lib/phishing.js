...
export function checkHost(host) {
    ...
    for (const good of KNOWN_GOOD_HOSTS) {
        ...
        if (candidate === goodNorm)
            continue; // exact normalised match would be verified above
        const d = editDistance(candidate, goodNorm);
        if (d > 0 && d <= 2) {
            reasons.push(`Domain "${root}" is visually similar to "${good}" (edit distance ${d}). Possible look-alike phishing.`);
            break;
        }
    }
    return { level: reasons.length > 0 ? 'suspicious' : 'unknown', reasons };
}
export function hostFromOriginSafe(origin) {
    try {
