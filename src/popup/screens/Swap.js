...
export default function Swap() {
    ...
    async function refreshQuote() {
        ...
        try {
            ...
            setQuote(q);
            if (!q)
                setQuoteErr('No Camelot route found for this pair');
        }
        catch (e) {
    ...
    const inUsd = useMemo(() => (tokenAUsd != null ? (inputNumber * tokenAUsd).toFixed(2) : null), [inputNumber, tokenAUsd]);
    ...
    const priceImpactPct = useMemo(() => {
        if (!quote || tokenAUsd == null || tokenBUsd == null)
            return null;
        const outN = parseFloat(quote.amountOutDisplay);
        if (inputNumber <= 0 || outN <= 0)
            return null;
    ...
    return (_jsxs(Screen, { children: [_jsx(TopBar, { title: "Swap", tone: "deck", right: _jsx("button", { onClick: () => setShowSettings(true), className: "hover:opacity-80", "aria-label": "Swap settings", children: _jsx("span", { role: "img", "aria-hidden": true, className: "block", style: {
                            ...
                            setPickerFor(null);
                                            ...
                                            } })] }), slippageBps >= 500 && (_jsx("div", { className: "text-warn text-xs mb-3", children: "High slippage \u2014 your trade may be sandwiched." })), _jsx("button", { className: "btn-primary w-full", onClick: () => setShowSettings(false), children: "Done" })] }) }))] }), _jsx(BottomNav, {}), txStatus !== 'idle' && (_jsx(TxStatus, { status: txStatus, message: txMessage, onDismiss: () => {
...
function Row({ label, value, muted, tone }) {
    ...
    return (_jsxs("div", { className: `flex justify-between ${muted ? 'text-ink-faint' : ''}`, children: [_jsx("span", { className: "text-ink-dim", children: label }), _jsx("span", { className: `font-mono ${valColor}`, children: value })] }));
}
